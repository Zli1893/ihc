...
ROOT_DIR = os.getcwd()
sys.path.append(os.path.join(ROOT_DIR, 'python-wsi-preprocessing-master'))
...
HSV_PURPLE = 270
HSV_PINK = 330
img_dir='/home/microway1/Data/TCGA-STAD/'
png_dir='/home/microway1/Data/TCGA-STAD-png/'
...
def generate_tile_summaries(tile_sum, np_img, display=True, save_summary=False):
  ...
  original_img_path = png_dir + slide_id.split('.svs')[0]+'.png'
  ...
  draw_orig.text((5, 5), summary_txt, SUMMARY_TITLE_TEXT_COLOR)
  ...
  #   plt.show()
    ...
    # summary_orig.show()
  if save_summary:
    save_tile_summary_image(summary, slide_id)
    save_tile_summary_on_original_image(summary_orig, slide_id)
...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
  ...
  for tile in tile_sum.tiles:
    if tile.score>0:
      all_score.append(tile.score)
  mean_score = np.mean(all_score)
  max_score = np.max(all_score)
  chose_tile = []
  for tile in tile_sum.tiles:
    if tile.score > (mean_score+max_score)/2.0:
      chose_tile.append(tile)
  top_tiles=chose_tile
  for t in top_tiles:
    border_color = tile_border_color(t.tissue_percentage)
    tile_border(draw, t.r_s + z, t.r_e + z, t.c_s, t.c_e, border_color)
    tile_border(draw_orig, t.r_s + z, t.r_e + z, t.c_s, t.c_e, border_color)
  ...
  for t in tiles_to_label:
    ...
    draw_orig.text(((t.c_s + h_ds_offset), (t.r_s + v_ds_offset + z)), label, (0, 0, 0))

    draw.text(((t.c_s + h_offset), (t.r_s + v_offset + z)), label, SUMMARY_TILE_TEXT_COLOR)
    draw_orig.text(((t.c_s + h_offset), (t.r_s + v_offset + z)), label, SUMMARY_TILE_TEXT_COLOR)
  ...
  #   summary_orig.show()
  if save_summary:
    save_top_tiles_image(summary, slide_id)
    save_top_tiles_on_original_image(summary_orig, slide_id)
  return summary,summary_orig
...
class TileSummary:
  ...
  def __init__(self, slide_id, orig_w, orig_h, orig_tile_w, orig_tile_h, scaled_w, scaled_h, scaled_tile_w,
  ...
  def display_summaries(self):
    ...
    """
    summary_and_tiles(self.slide_id, display=True, save_summary=False, save_data=False, save_top_tiles=False)
